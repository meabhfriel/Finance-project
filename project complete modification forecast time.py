...
def capex_forecast_prompt(ticker: str, df_with_fundamentals: pd.DataFrame) -> str:
    ...
    4. Output the projected CapEx values **as a list of 5 numbers**, in plain decimal format, without dollar signs or commas and as full numbers.
...
capex_forecast_output = call_ollama(capex_forecast_prompt(selected_ticker, fundamental_data))
...
def da_forecast_prompt(ticker: str, df_with_fundamentals: pd.DataFrame) -> str:
    ...
    4. Output only the projected D&A values **as a list of 5 numbers**, in plain decimal format, and without symbols or commas and as full numbers.
...
da_forecast_output = call_ollama(da_forecast_prompt(selected_ticker, fundamental_data))
print("AI Response:\n", da_forecast_output)
...
def ebit_forecast_prompt(ticker: str, df_with_fundamentals: pd.DataFrame) -> str:
    ...
    4. Output the projected EBIT values **as a list of 5 integers**, one per year, in standard notation (no symbols or commas) and as full numbers.
...
ebit_forecast_output = call_ollama(ebit_forecast_prompt(selected_ticker, fundamental_data))
...
def wc_forecast_prompt(ticker: str, df_with_fundamentals: pd.DataFrame) -> str:
    ...
    4. Output the projected ΔNWC values **as a list of 5 numbers**, in plain decimal format and as full numbers.
...
wc_forecast_output = call_ollama(wc_forecast_prompt(selected_ticker, fundamental_data))
print("AI Response:\n", wc_forecast_output)
